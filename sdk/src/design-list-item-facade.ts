...
export class DesignListItemFacade {
  ...
  /** @internal */
  setFontSource(fontSource: FontSource): void {
   ...
   */
  async fetchDesign(options: {
    cancelToken?: CancelToken | null
  }): Promise<DesignFacade> {
    const design = await this._sdk.fetchDesignById(this.id, {
      ...options,
      designVersionId: this.versionId,
    })

    if (this._fontSource) {
      design.setFontSource(this._fontSource)
    }
